...
const Header = () => {
	...
	const { getUser, setGetUser, getUserFnc } = useData();

	const handleLogout = async () => {
		const logout = authServices.logoutUser();
				...
				<nav className="max-w-7xl mx-auto px-5 py-3 flex justify-between items-center">
					<div className="logo">
						<Link href={"/"} className="flex items-end gap-1">
							...
							<h5 className="text-xl">File Share</h5>
						</Link>
					</div>
							...
							{getUser ? (
								...
								<>
									...
									<li>
										<Link
											href={"/signup"}
											className="text-sm bg-slate-900 text-white px-3 py-3 rounded-md"
										>
											Signup
										</Link>
									</li>
